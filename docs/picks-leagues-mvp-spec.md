...
| 2026-05-25 | FND-012 | Vercel serverless function entry moved from `services/api/api/index.ts` to `api/index.ts` at the repo root. Rationale: single-project deploy (SPA + API co-deployed) requires the function to be discovered at the deployment root `api/` directory. The old path was scoped to the API sub-package under the FND-005 assumption of a separate Vercel project per package. |
| 2026-05-25 | FND-012 | Vercel function switched from TypeScript source at `api/index.ts` to a pre-bundled ESM file produced by esbuild (`pnpm build:api` → `api/index.js`). The source entry is now `services/api/src/vercel-entry.ts` (included in the normal `services/api` typecheck). Rationale: Vercel's `@vercel/node` builder hard-codes `moduleResolution: nodenext`, which conflicts with our `moduleResolution: bundler` tsconfig and rejects relative imports without `.js` extensions — causing the function to silently fail to deploy despite a "Ready" build status. Pre-bundling bypasses Vercel's TypeScript compilation entirely. |
| 2026-05-25 | FND-012 | Bundle output renamed from `api/index.js` to `api/[[...slug]].js`. Rationale: Vercel's filesystem routing maps `api/index.js` to the literal URL `/api` only — `/api/*` subpaths returned a 404 before reaching Hono. The `[[...slug]]` optional catch-all filename tells Vercel to match `/api` and any path beneath it and forward the full URL to the function so Hono can dispatch internally. |
| 2026-05-25 | FND-012 | API entry point switched from `hono/vercel` to `@hono/node-server` (`getRequestListener`). Rationale: `hono/vercel`'s `handle()` expects Fetch API `Request`, but Vercel's generic Node runtime invokes serverless functions with Node `IncomingMessage`/`ServerResponse`, causing `c.req.header()` to fail at runtime. `getRequestListener` bridges the two APIs. The entry point is now in `services/api/src/vercel-entry.ts` and bundled by esbuild. |
| 2026-05-26 | hotfix | Switched from Vercel filesystem auto-detection to the Build Output API (`scripts/build-vercel-output.sh` → `.vercel/output/`). Symptom: after FND-012 merged, CI/CD builds produced a `Ready` status but Vercel's Builds section was empty — the bundled function (`api/[[...slug]].js`) was never registered because auto-detection does not pick up build-time-generated files from a git-sourced build. Fix: `buildCommand` now runs `pnpm vercel:build`, which writes `.vercel/output/` directly in Build Output API v3 format; Vercel reads that directory definitively with no auto-detection. The old `build:api` script and `api/` root directory are removed. |
